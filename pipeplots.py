            ...
            if couP<nplots:
                ...
                for base in list(foo1.baseline.unique()):
                    ...
                    if ('ALMA' in base) or (('SMA' in base)&('JCMT' not in base)):
                        ax[couR,couC].errorbar(foo2.uvdist,foo2.amp,bars_on*foo2.sigma,fmt='o',mfc='none',ms=12,color=palette_dict[base],label=base)
                    else:
                        ax[couR,couC].errorbar(foo2.uvdist,foo2.amp,bars_on*foo2.sigma,fmt='x',ms=10,color=palette_dict[base],label=base)
                if (couR==0)&(couC==0):
